#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Copyright 2004, 2005 R�man Joost <roman@bromeco.de> - Rotterdam, Netherlands
...
import rocon_python_wifi.flags
from rocon_python_wifi.iwlibs import Wireless, Iwrange, getNICnames
...
# "Check if the interface could support scanning"
    try:
        ...
        try:
            ...
            if e.error_number != errno.EPERM:
                sys.stderr.write(
                    "%-8.16s  Interface doesn't support scanning : %s\n\n" %
                    (wifi.ifname, e.error_string))
        else:
            if (len(results) == 0):
...
def print_channels(wifi, args=None):
    ...
    try:
        ...
        if (e.error_number == errno.EOPNOTSUPP) or \
           (e.error_number == errno.EINVAL) or \
           (e.error_number == errno.ENODEV):
            sys.stderr.write("%-8.16s  no frequency information.\n\n" % (
                                wifi.ifname, ))
        else:
            report_error("channel", wifi.ifname, e.error_number, e.error_string)
    else:
    ...
    try:
        ...
        if (e.error_number == errno.EOPNOTSUPP) or \
           ...
           (e.error_number == errno.ENODEV):
            ...
            report_error("bit rate", wifi.ifname, e.error_number, e.error_string)
    else:
...
def print_power(wifi, args=None):
    ...
    try:
        ...
        if (e.error_number == errno.ENODEV):
